...
public class VoxelState {
        ...
        set {
            if (value != _light) {
                ...
                if (_light < oldLightValue) {
                    Queue<int> neigboursToDarken = new Queue<int>();
                    ...
                    // while (neigboursToDarken.Count > 0) {
                    //     neighbours[neighborsToDarken.Dequeue()].light = 0;
                    // }
    ...
    public byte castLight {
        get {
            // Get the amount of light this voxel is spreading. Bytes wrap around so we
    ...
    public void PropagateLight() {
        // If we somehow added a null voxel or one that isn't bright enough to propagate, return
        if (light < 2)
            return;
        // Loop though each neighbour of this voxel
        for (int p = 0; p < 6; p++) {
