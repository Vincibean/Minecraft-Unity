...
public class Clouds : MonoBehaviour {
    ...
    private void CreateClouds() {
        for (int x = 0; x < cloudTexWidth; x += cloudTileSize) {
            for (int y = 0; y < cloudTexWidth; y += cloudTileSize) {
                ...
                clouds.Add(CloudTilePosFromV3(position), CreateCloudTile(CreateCloudMeshData(x, y), position));                ;
            }
        }
    }
    ...
    public void UpdateClouds() {
        for (int x = 0; x < cloudTexWidth; x += cloudTileSize) {
            for (int y = 0; y < cloudTexWidth; y += cloudTileSize) {
                ...
                position = new Vector3(RoundToCloud(position.x), cloudHeight, RoundToCloud(position.x));
    ...
    private Mesh CreateCloudMeshData(int x, int z) {
        ...
        for (int xIncrement = 0; xIncrement < cloudTileSize; xIncrement++) {
            for (int zIncrement = 0; zIncrement < cloudTileSize; zIncrement++) {
                ...
                if (cloudData[xVal, zVal]) {
                    ...
                    // Note: this are local (i.e. relative) values; that's why y = 0
                    // This y is relative to cloudHeight; in other words, this will be
                    // leveled with cloudHeight
    ...
    private int CloudTileCoordFromFloat(float value) {
        float a = value / (float)cloudTexWidth; // Gets the position using cloud texture width as units.
        a -= Mathf.FloorToInt(a); // Subtract whole number to get a 0-1 value to represent position in cloud texture
        int b = Mathf.FloorToInt((float)cloudTexWidth * a); // Multiply cloud texture width by a to get position in texture gloally
